...
def is_ranges_overlapping(range1: tuple, range2: tuple):
    return is_one_range_contained_in_other(range1, range2) or range1[0] <= range2[0] <= range1[1] or range1[0] <= range2[1] <= range1[1]
...
# (range1, range2) = range_string_to_tuples( "1-3,4-6" )
# print(is_ranges_overlapping(range1, range2))

with open(r"input.txt", "r") as input:
    for line in input.readlines():
        (range1, range2) = range_string_to_tuples( line.strip() )
        if is_ranges_overlapping(range1, range2):
            total += 1
